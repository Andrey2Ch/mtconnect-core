...
try {
    ...
    console.log('⚠️ FOCAS addon не найден, будет использоваться эмуляция:', (error as Error).message);
}
...
export class RealFocasClient {
    ...
    async connect(ip: string, port: number = 8193): Promise<FocasConnectionResult> {
    ...
    async disconnect(ip: string): Promise<boolean> {
        if (!focasAddon) {
            return false;
        }
    ...
    async readMachineData(ip: string): Promise<FocasMachineData> {
        if (!focasAddon) {
    ...
    async getMachineData(machineId: string, ip: string, port: number = 8193): Promise<FocasData> {
        if (!this.isAvailable()) {
    ...
    private async getMachineDataViaAddon(machineId: string, ip: string, port: number): Promise<FocasData> {
        try {
    ...
    private async getMachineDataViaProcess(machineId: string, ip: string, port: number): Promise<FocasData> {
    ...
    private async executeLadder99Driver(machineId: string, ip: string, port: number, exePath: string): Promise<FocasData> {
        const { spawn } = require('child_process');
...
`;
    ...
    private async executeQuickFocasDriver(machineId: string, ip: string, port: number, exePath: string): Promise<FocasData> {
        const { spawn } = require('child_process');
    ...
    private parseProcessOutput(output: string): any {
        const data = {
            running: false,
            feedrate: 0,
            spindleSpeed: 0,
    ...
    private parseLadder99Output(output: string): any {
        const data = {
            running: false,
            feedrate: 0,
            spindleSpeed: 0,
                ...
                if (jsonData.machines && typeof jsonData.machines === 'object') {
                    ...
                    if (machine) {
                        ...
                        if (machine.alarms && Array.isArray(machine.alarms)) {
                            data.alarms = machine.alarms.map((alarm: any) => 
                                typeof alarm === 'string' ? alarm : alarm.message || 'Unknown alarm'
                            );
                        }
                    }
                }
    ...
    private getErrorResult(machineId: string, error: string): FocasData {
        return {
            machineId,
            status: {
                running: false,
    ...
    private determineRunningStatus(status: any): boolean {
    ...
    async testConnection(ip: string, port: number = 8193): Promise<boolean> {
        if (!this.isAvailable()) {
            return false;
        }
...
export const realFocasClient = new RealFocasClient();
