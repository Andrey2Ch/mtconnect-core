...
Write-Host "MTConnect System Launcher v1.1 (Fixed)" -ForegroundColor Cyan
...
# Start Edge Gateway with proper PowerShell command
$startScript = @"
`$env:CLOUD_API_URL='$CloudApiUrl'
`$env:EDGE_GATEWAY_ID='$($env:COMPUTERNAME)-edge-gateway'
Set-Location '$PWD'
...
$startScript | Out-File -FilePath "start-edge-gateway.ps1" -Encoding UTF8
Start-Process -FilePath "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass", "-File", "start-edge-gateway.ps1" -WindowStyle Minimized
...
$apiStatus = "ERROR"
try {
    # Check if port 3555 is listening first
...
Write-Host "This window will close automatically in 10 seconds..." -ForegroundColor Yellow
...
Remove-Item "start-edge-gateway.ps1" -ErrorAction SilentlyContinue 